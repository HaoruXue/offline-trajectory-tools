...
class Trajectory:
    def __init__(self, num_point: int) -> None:
        self.points = np.zeros((num_point, 12), dtype=np.float64)
        self.points[:, TrajectoryPoint.IDX] = np.arange(0, len(self.points), 1)
    ...
    def fill_curvature(self):
        for i in range(len(self.points)):
            ...
            if c >= len(self.points):
                c = 0
            self.points[i, TrajectoryPoint.CURVATURE] = Trajectory.get_curvature_from_three_points(
